...
class StrictTgapipldcLocatorService(TgapipldcLocatorService):
    ...
    def __init__(self, *args, proxy_provider: ProxyProvider | None = None, **kwargs) -> None:
        self._strict_proxy_provider = proxy_provider
        super().__init__(*args, **kwargs)
        if self._strict_proxy_provider is None:
            self._strict_proxy_provider = self._discover_proxy_provider()
    ...
    def proxy_for_profile(self, profile_dir: str) -> str:
        provider = self._strict_proxy_provider or self._discover_proxy_provider()
        if provider is None:
