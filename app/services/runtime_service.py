...
from app.services.user_state_service import UserStateStore
...
class RuntimeService(QObject):
    ...
    def __init__(self, base_dir: str = "."):
        ...
        self.accounts, self.rules, self.templates, self.settings = self.config_service.load_all()

        self._apply_runtime_paths()

        self.logger = setup_logger(self.settings)
        self.user_state_store = UserStateStore(str(self.config_service.users_path))
        ...
        self._last_templates_signature: tuple[tuple[str, str, tuple[int, ...], bool], ...] = (
            self._build_templates_signature(self.templates)
        )

    def _build_templates_signature(self, templates) -> tuple[tuple[str, str, tuple[int, ...], bool], ...]:
        return tuple(
            (
    ...
    def reload_config_cache(self) -> None:
        ...
        if self._manager is not None:
            self._manager.update_configuration(self.accounts, self.rules, self.settings)

    def sync_templates_from_disk(self) -> bool:
        """
        仅同步 templates.json，不弹窗、不重载全部配置。
        返回值:
            True  -> 模板列表有变化
            False -> 模板列表无变化
        """
        _, _, latest_templates, _ = self.config_service.load_all()
    ...
    def save_rules(self, rules) -> None:
        self.rules = rules
        self.config_service.save_rules(rules)
    ...
    def shutdown(self) -> None:
        if self._manager is not None and self._loop is not None:
            try:
                future = asyncio.run_coroutine_threadsafe(self._manager.stop_all(), self._loop)
