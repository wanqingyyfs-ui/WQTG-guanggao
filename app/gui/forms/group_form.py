...
class GroupForm(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.group_group_edit = QLineEdit()
        self.group_group_edit.setPlaceholderText("可选，多个群聊组用逗号分隔；推荐在“分组管理”页维护")
    ...
    def _build_ui(self) -> None:
        ...
        self._add_labeled_widget(grid, 1, 0, "所属群聊组：", self.group_group_edit)
        self._add_labeled_widget(grid, 2, 0, "Chat ID：", self.chat_id_edit)
        self._add_labeled_widget(grid, 2, 2, "Username：", self.username_edit)

        label = QLabel("备注：")
        label.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTop)
        grid.addWidget(label, 3, 0)
        grid.addWidget(self.remark_edit, 3, 1, 1, 3)
    ...
    def _add_labeled_widget(grid: QGridLayout, row: int, label_column: int, label_text: str, widget: QWidget) -> None:
        ...
        grid.addWidget(widget, row, label_column + 1, 1, 3 if label_column == 0 and row == 1 else 1)
    ...
    def load_group(self, group: GroupConfig) -> None:
        ...
        self.group_group_edit.setText("、".join(self._group_group_names(group)))
    ...
    def get_form_group(self) -> GroupConfig:
        ...
        username = self._normalize_username(self.username_edit.text()) if self.default_group_username_normalize else self.username_edit.text().strip()
        group_group_names = self._parse_group_group_names(self.group_group_edit.text())
        return GroupConfig(
            group_id=group_id,
            ...
            group_group=group_group_names[0] if group_group_names else "",
            group_group_names=group_group_names,
        )

    @staticmethod
    def _parse_group_group_names(value: Any) -> list[str]:
